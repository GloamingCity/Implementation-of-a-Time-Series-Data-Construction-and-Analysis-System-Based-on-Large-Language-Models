...
def extract_series_from_tscapeval(sample):
...
def main():
    parser = argparse.ArgumentParser(description="ts-caption-eval 批量推理脚本")
    parser.add_argument("--checkpoint", type=str, required=True, help="checkpoint 路径")
    parser.add_argument("--input_dir", type=str, required=True, help="ts-caption-eval 数据目录")
    ...
    print(f"[TSCAPEVAL_INFER] device={device}, precision={precision_mode}")
    ...
    data_files = [input_dir / "ett.jsonl", input_dir / "fred.jsonl", input_dir / "nab.jsonl", input_dir / "ucr.jsonl"]
    ...
    for data_file in data_files:
        print(f"[TSCAPEVAL_INFER] 处理数据集: {data_file.name}")
        ...
        for idx, sample in enumerate(samples):
            ...
            series_name = sample.get("series_name", "value")
            ts_values = extract_series_from_tscapeval(sample)
            ...
            if (idx + 1) % args.log_interval == 0 or (idx + 1) == len(samples):
                print(f"[TSCAPEVAL_INFER] {data_file.name}: {idx + 1}/{len(samples)} 样本已处理")
    ...
    print(f"[TSCAPEVAL_INFER] 完成！共处理 {total_samples} 个样本")
    print(f"[TSCAPEVAL_INFER] 预测文件已保存到: {output_path}")
