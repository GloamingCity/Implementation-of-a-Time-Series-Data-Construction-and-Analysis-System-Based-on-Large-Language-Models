...
def main():
    ...
    print(f"[TSCAPEVAL_INFER] 加载 tokenizer: {args.model_path}")
    tokenizer = load_tokenizer(args.model_path)

    print(f"[TSCAPEVAL_INFER] 构建 encoder: {args.encoder_type}")
    encoder = build_encoder(args.encoder_type, ts_seq_len=args.ts_seq_len)

    print(f"[TSCAPEVAL_INFER] 加载模型...")
    ...
    print(f"[TSCAPEVAL_INFER] 加载权重: {ckpt_path}")
    ...
    print(f"[TSCAPEVAL_INFER] 推理参数: max_new_tokens={max_new_tokens}, temperature={temperature}, repetition_penalty={repetition_penalty}, no_repeat_ngram_size={no_repeat_ngram_size}")

    data_files = [
        input_dir / "ett.jsonl",
        input_dir / "fred.jsonl",
        input_dir / "nab.jsonl",
        input_dir / "ucr.jsonl",
    ]
    ...
    if not data_files:
        raise FileNotFoundError(f"未找到数据文件: {input_dir}/*.jsonl")
    ...
    for data_file in data_files:
        ...
        # 测试模式：每个数据集仅处理前N个样本
        per_dataset_sample_size = args.sample_size if args.sample_size > 0 else 0
        if per_dataset_sample_size > 0:
            samples = samples[:per_dataset_sample_size]
            print(f"[TSCAPEVAL_INFER] 测试模式：每个数据集仅处理前 {per_dataset_sample_size} 个样本")
