...
DOUBAO_QUOTA_HTTP_STATUS = {402, 429}
...
UPSTREAM_CONCURRENCY_WITH_DOUBAO = 15
UPSTREAM_CONCURRENCY_WITHOUT_DOUBAO = 7
...
REFERENCE_POOL_SHARED_MODEL_GROUPS = {
    ...
    # 豆包与智谱/硅基流动分离，维护独立提示样本池。
    "doubao_seed_20_lite_260215": "doubao_only",
}
...
REFERENCE_UPDATE_MIN_SCORE = 85
DOUBAO_REFERENCE_UPDATE_MIN_SCORE = 88
...
# 模型评分阈值建议（2026-04-14 豆包实测：keep>=72, drop<=62；保留阈值按包含边界执行）。
DOUBAO_RECOMMENDED_SCORE_THRESHOLD = 72
...
def _has_doubao_model_configured() -> bool:
    return any(str(cfg.get("provider") or "").strip().lower() == "doubao" for cfg in MODEL_FALLBACKS)
...
def _refresh_max_concurrency_hint() -> None:
    ...
    with _DOUBAO_QUOTA_LOCK:
        doubao_exhausted = bool(_DOUBAO_QUOTA_EXHAUSTED)

    target_hint = (
        int(UPSTREAM_CONCURRENCY_WITHOUT_DOUBAO)
        if doubao_exhausted
        else int(UPSTREAM_CONCURRENCY_WITH_DOUBAO)
    )
...
_DOUBAO_QUOTA_EXHAUSTED = False
_DOUBAO_QUOTA_EXHAUST_REASON = ""
_DOUBAO_NEXT_RECHECK_TS = 0.0
_DOUBAO_RECHECK_IN_PROGRESS = False
...
def _rebuild_runtime_model_states(reset_doubao_quota: bool = False):
    ...
    global _DOUBAO_QUOTA_EXHAUSTED, _DOUBAO_QUOTA_EXHAUST_REASON
    global _DOUBAO_NEXT_RECHECK_TS, _DOUBAO_RECHECK_IN_PROGRESS
...
def _is_doubao_quota_exhausted() -> bool:
    with _DOUBAO_QUOTA_LOCK:
        return bool(_DOUBAO_QUOTA_EXHAUSTED)


def _find_first_doubao_model() -> dict | None:
    for cfg in MODEL_FALLBACKS:
        if _provider_name(cfg) == "doubao":
            return cfg
    return None
...
def _mark_doubao_quota_exhausted(reason: str, request_tag: str):
    global _DOUBAO_QUOTA_EXHAUSTED, _DOUBAO_QUOTA_EXHAUST_REASON, _DOUBAO_NEXT_RECHECK_TS
    if not DOUBAO_QUOTA_FALLBACK_TO_ZHIPU:
        return

    notify = False
    with _DOUBAO_QUOTA_LOCK:
        if not _DOUBAO_QUOTA_EXHAUSTED:
            notify = True
        _DOUBAO_QUOTA_EXHAUSTED = True
        _DOUBAO_QUOTA_EXHAUST_REASON = str(reason or "").strip()[:240]
        _DOUBAO_NEXT_RECHECK_TS = time.time() + float(DOUBAO_QUOTA_RECHECK_INTERVAL_SEC)
    ...
    if notify:
        print(
            "提示: "
            f"{request_tag} 检测到豆包额度不足/耗尽，后续请求将动态分配到其它可用API。"
            f"上游并发提示已切换到 {int(UPSTREAM_CONCURRENCY_WITHOUT_DOUBAO)}。"
            f"将每 {int(DOUBAO_QUOTA_RECHECK_INTERVAL_SEC // 60)} 分钟自动重试豆包可用性。"
            f"触发原因: {_DOUBAO_QUOTA_EXHAUST_REASON}"
        )


def _probe_doubao_availability(timeout_sec: int, request_tag: str) -> tuple[bool, bool, str]:
    cfg = _find_first_doubao_model()
    if cfg is None:
        return False, False, "未配置豆包模型"
    ...
    if is_quota_limited and detail:
        _mark_doubao_quota_exhausted(detail, request_tag)
    return False, is_quota_limited, detail


def _try_reactivate_doubao_if_due():
    global _DOUBAO_RECHECK_IN_PROGRESS, _DOUBAO_QUOTA_EXHAUSTED
    global _DOUBAO_QUOTA_EXHAUST_REASON, _DOUBAO_NEXT_RECHECK_TS

    if not DOUBAO_QUOTA_FALLBACK_TO_ZHIPU:
        return
    if _find_first_doubao_model() is None:
        return
    ...
    with _DOUBAO_QUOTA_LOCK:
        if not _DOUBAO_QUOTA_EXHAUSTED:
            return
        if _DOUBAO_RECHECK_IN_PROGRESS:
            return
        if now < float(_DOUBAO_NEXT_RECHECK_TS):
            return
        _DOUBAO_RECHECK_IN_PROGRESS = True
        _DOUBAO_NEXT_RECHECK_TS = now + float(DOUBAO_QUOTA_RECHECK_INTERVAL_SEC)

    try:
        ok, quota_limited, detail = _probe_doubao_availability(
            timeout_sec=int(DOUBAO_QUOTA_RECHECK_TIMEOUT_SEC),
            request_tag="doubao_hourly_recheck",
        )
        ...
        if ok:
            ...
            _refresh_max_concurrency_hint()
            print(
                "提示: 豆包API额度重试成功，已恢复加入动态调度。"
                f"上游并发提示已恢复到 {int(UPSTREAM_CONCURRENCY_WITH_DOUBAO)}。"
            )
        elif quota_limited:
            with _DOUBAO_QUOTA_LOCK:
                _DOUBAO_QUOTA_EXHAUSTED = True
                if detail:
                    _DOUBAO_QUOTA_EXHAUST_REASON = detail[:240]
            _refresh_max_concurrency_hint()
            print("提示: 豆包API额度仍受限，继续使用其它API；下一次重试将在1小时后。")
        else:
            with _DOUBAO_QUOTA_LOCK:
                _DOUBAO_QUOTA_EXHAUSTED = True
                if detail:
                    _DOUBAO_QUOTA_EXHAUST_REASON = detail[:240]
            _refresh_max_concurrency_hint()
            print("提示: 豆包API每小时重试失败（非额度错误），本轮继续回退到其它API。")
    finally:
        with _DOUBAO_QUOTA_LOCK:
            _DOUBAO_RECHECK_IN_PROGRESS = False
...
def _startup_probe_doubao_quota():
    ...
    print("启动探测: 尝试调用豆包API以确认当前额度状态。")
    ok, quota_limited, detail = _probe_doubao_availability(
        timeout_sec=int(DOUBAO_QUOTA_RECHECK_TIMEOUT_SEC),
        request_tag="doubao_startup_probe",
    )
    ...
    print(f"启动探测结果: 豆包API当前不可用（非额度限制），将继续保留并在运行中重试。detail={detail}")


def _active_models_for_request() -> list[dict]:
    _try_reactivate_doubao_if_due()
    disable_doubao = _is_doubao_quota_exhausted()
    _refresh_max_concurrency_hint()
    active_models = []
    for cfg in MODEL_FALLBACKS:
        if disable_doubao and _provider_name(cfg) == "doubao":
            continue
...
def _call_with_tiered_failover(call_fn, failover_state: dict, request_tag: str):
    ...
    if not active_models:
        if _is_doubao_quota_exhausted():
            raise RuntimeError("豆包额度已耗尽，且当前未配置其它可用模型。")
    ...
    for mcfg in dispatch_order:
        ...
        try:
            ...
            if provider == "doubao" and _is_doubao_quota_exhausted_error(model_e):
                _mark_doubao_quota_exhausted(err_msg, request_tag)
...
def _ensure_type_reference_cache(norm_type: str, model_id: str | None = None) -> list[dict]:
    ...
    cached = _normalize_reference_items(good_items + legacy_items)
...
def _prune_reference_pool(items: list[dict], max_candidates: int) -> list[dict]:
    ...
    ranked = sorted(
        normalized,
        key=lambda d: (_item_score(d), str(d.get("label") or "")),
        reverse=True,
    )
...
def _pick_reference_items_with_fixed_count(
    ...
    """Keep type references at a fixed count; sample pool randomly and backfill if needed."""
    ...
    def _append_with_constraints(relax_dataset: bool, relax_source: bool) -> None:
        nonlocal candidates
        remain = []
    ...
    _append_with_constraints(relax_dataset=False, relax_source=False)
    if len(chosen) < target:
        _append_with_constraints(relax_dataset=True, relax_source=False)
    if len(chosen) < target:
        _append_with_constraints(relax_dataset=True, relax_source=True)

    # 如果依旧不足6张，则允许从已选项中随机复用，确保提示样本数量不下降。
...
def get_type_reference_items(
    ...
    base_limit = int(TYPE_REFERENCE_LIMITS.get(norm_type, TYPE_REFERENCE_TARGET_COUNT))
...
def get_batch_image_scores(
    ...
    siliconflow_ref_cap = max(3, int(SILICONFLOW_BATCH_MAX_REFERENCE_IMAGES))
            ...
            try:
                def _batch_call_one(mcfg: dict):
                    effective_reference_items = get_type_reference_items(
                        ...
                        model_id=_reference_pool_key_for_model_cfg(mcfg),
                    )
...
def main():
    ...
    if _has_doubao_model_configured():
        print(
            "上游并发策略: "
            f"豆包可用时={UPSTREAM_CONCURRENCY_WITH_DOUBAO}, "
            f"豆包停用时={UPSTREAM_CONCURRENCY_WITHOUT_DOUBAO}, "
