...
def load_tokenizer(model_path: str):
    try:
        tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=True, use_fast=False, fix_mistral_regex=True, local_files_only=True)
    except Exception:
...
def build_prompt(tokenizer, ts_tensor, series_name="value", enforce_relative_time_no_meta=True, encoder_type="cnn"):
    prompt_messages, prompt_meta = build_adaptive_prompt_messages(ts_tensor=ts_tensor, force_detail_level=None)

    time_rule = (
        "时间约束：当前样本不含可验证绝对时间锚点。禁止生成具体日期或时刻；统一使用\"第k个时间点/第a-b个时间点\"表达。"
        if bool(enforce_relative_time_no_meta) else
        "时间约束：若缺少可验证时间锚点，请谨慎使用时间表达，避免编造。"
    )
    series_rule = f"变量名约束：全文主变量名必须严格写成\"{series_name}\"，不要使用近似拼写、错别字、缩写或连字符变体。"
    ...
    prompt_messages = list(prompt_messages) + [{"role": "user", "content": f"{time_rule}\n{series_rule}\n{structure_rule}"}]
...
def main():
    ...
    model = MultimodalQwen(encoder=encoder, qwen_model_path=args.model_path, llm_dtype=compute_dtype, soft_prompt_len=args.soft_prompt_len, use_gating=True, use_bridge_norm=True)
    ...
    encoder_params = {
        "cnn": {"max_new_tokens": 500, "min_new_tokens_base": 150, "temperature": 0.7, "repetition_penalty": 1.1, "no_repeat_ngram_size": 6},
        "mlp": {"max_new_tokens": 600, "min_new_tokens_base": 200, "temperature": 0.1, "repetition_penalty": 1.2, "no_repeat_ngram_size": 10},
        "patchtst": {"max_new_tokens": 700, "min_new_tokens_base": 200, "temperature": 0.2, "repetition_penalty": 1.25, "no_repeat_ngram_size": 10},
    }
    ...
    for idx, sample in enumerate(samples):
        ...
        gen_kwargs = {
            ...
            "no_repeat_ngram_size": params.get("no_repeat_ngram_size", 6),
        }
        ...
        with torch.no_grad(), autocast_ctx:
            mm_ids = model.generate(ts_data=ts_tensor, input_ids=input_ids, attention_mask=attention_mask, **gen_kwargs)

        pred_caption = decode_generated_text(tokenizer, mm_ids, prompt_token_len=input_ids.size(1), extra_prefix_tokens=args.soft_prompt_len)
        all_predictions.append({"ts_id": ts_id, "dataset": dataset, "pred_caption": pred_caption})
    ...
    print(f"[QA_INFER] 完成！共处理 {len(all_predictions)} 个样本")
