...
class MultimodalQwen(nn.Module):
    def __init__(
        ...
        # 尝试读取实际 hidden_size 并打印，便于确认是否加载了预期基座模型
        try:
    ...
    def _encode_ts_soft_prompt(self, ts_data):
        # 跨数据集量纲差异很大，先做数值防护，避免极端值导致训练不稳定。
    ...
    def forward(self, ts_data, input_ids, attention_mask=None, labels=None):
        ...
        if labels is not None:
            ...
            full_labels = torch.cat([prefix_labels, labels], dim=1)  # -> (B, K+L)

        # 7) 仅通过 inputs_embeds 送入 CausalLM 计算 loss / logits
        outputs = self.llm(
