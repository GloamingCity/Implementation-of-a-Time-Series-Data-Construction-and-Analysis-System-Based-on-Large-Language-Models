...
def resolve_relative_path(path_str: str, base_dir: Path) -> Path:
    """将传入路径解析为绝对路径：相对路径按 base_dir 解释。"""
    p = Path(path_str)
    if p.is_absolute():
        return p
...
def merge_filtered_samples_jsonl(sample_run_dir: Path, combined_jsonl_path: Path) -> dict:
    """
    将 sample_run_dir 下各类型目录中的 samples_filtered.jsonl 合并为一个 JSONL。

    说明：
    - 仅合并一层子目录下的 samples_filtered.jsonl，例如
      sample_run_dir/anomaly_detection/samples_filtered.jsonl。
    - 为规避并发写入时尾行不完整问题，逐行做 JSON 校验，坏行会跳过并计数。
    - 采用临时文件 + 原子替换，避免读到半成品合并文件。
    """
...
def resolve_precision(device, precision_arg):
    """
    训练精度策略：
    - auto: CUDA 且支持 BF16 则用 BF16，否则 FP16
    - bf16/fp16/fp32: 按用户指定；若 bf16 不支持则自动回退到 fp16
    返回: (precision_mode, compute_dtype, use_amp, use_grad_scaler)
    """
    if device.type != "cuda":
...
def build_checkpoint_stage_dir(train_dir, training_stage):
    """
    根据训练阶段返回 checkpoint 目录：
    - 冻结阶段: Train/Checkpoints/Frozen_LLM_Stage
    - LoRA 阶段: Train/Checkpoints/Unfrozen_LoRA_Stage

    兼容迁移：如果历史目录 1_Frozen_LLM_Stage / 2_Unfrozen_LoRA_Stage 存在，
    自动把其中内容迁移到新目录。
    """
...
def extract_trainable_state_dict(model):
    """
    只导出 requires_grad=True 的参数，避免保存冻结的 LLM 全量权重。
    """
    trainable = {}
...
def maybe_save_lora_adapter(model, output_dir):
    """
    若当前 LLM 已挂载 LoRA/PEFT 适配器，则导出 adapter 文件：
    - adapter_config.json
...
def main():
    ...
    parser.add_argument(
        ...
        help="合并后的训练 JSONL 文件名（写入 sample_run_dir 下）",
    )
    args = parser.parse_args()

    args.model_path = str(resolve_relative_path(args.model_path, base_dir=project_root))
    sample_run_dir = resolve_relative_path(args.sample_run_dir, base_dir=project_root)

    # 若未显式指定 train_jsonl，则将样本目录下各类型 jsonl 合并后作为训练集。
    if args.train_jsonl is None:
        combined_jsonl = sample_run_dir / str(args.combined_jsonl_name)
        merge_stats = merge_filtered_samples_jsonl(
            sample_run_dir=sample_run_dir,
            combined_jsonl_path=combined_jsonl,
        )
        args.train_jsonl = str(combined_jsonl)
        print(f"[样本合并] 合并后的训练集: {args.train_jsonl}")
        print(
            "[样本合并] "
            f"来源文件数={merge_stats['source_file_count']} "
            f"写入样本数={merge_stats['written_lines']} "
            f"跳过坏行={merge_stats['invalid_lines']}"
        )
        if merge_stats["changed_files"]:
            print("[WARN] 检测到以下源文件在合并期间发生变化，结果可能不是严格快照：")
            for p in merge_stats["changed_files"]:
                print(f"[WARN]   {p}")
            print("[WARN] 若追求严格一致，建议暂停样本生成后再执行一次合并。")
    else:
        args.train_jsonl = str(resolve_relative_path(args.train_jsonl, base_dir=project_root))
    
    # 确保 train_jsonl 被指定
    if args.train_jsonl is None:
        raise ValueError("请指定 --train_jsonl，或确保 --sample_run_dir 下存在可合并样本。")

    if not Path(args.train_jsonl).exists():
        raise FileNotFoundError(f"训练集 JSONL 不存在: {args.train_jsonl}")
    ...
    train_dataset = JsonlTimeSeriesDataset(
    ...
    projection_lr = args.projection_lr if args.projection_lr is not None else (args.lr * 3.0)
    ...
    trainable_params = encoder_params + bridge_params
    optimizer = optim.AdamW(
        [
            {"params": encoder_params, "lr": encoder_lr},
            {"params": bridge_params, "lr": projection_lr},
        ],
        weight_decay=args.weight_decay,
    )

    print(f"[INFO] 模型初始化完成，共有 {sum(p.numel() for p in trainable_params)} 个参数需要训练。")
    print(f"[INFO] 训练样本数: {len(train_dataset)}")
    print(f"[INFO] 当前设备: {device}")
    print(f"[INFO] 训练精度: {precision_mode} (dtype={compute_dtype}, AMP={use_amp})")
    print(f"[INFO] soft_prompt_len: {args.soft_prompt_len}")
    print(f"[INFO] use_gating: {args.use_gating}")
    print(f"[INFO] use_bridge_norm: {args.use_bridge_norm}")
    print(f"[INFO] 分组学习率: encoder={encoder_lr}, bridge={projection_lr}")
    print(f"[INFO] Checkpoint 目录: {checkpoint_stage_dir}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in progress_bar:
            ts_data = batch["ts_data"].to(device, non_blocking=True)
            ...
            if not torch.isfinite(loss):
                ...
                optimizer.zero_grad(set_to_none=True)
                if nonfinite_loss_steps <= 3 or step % args.log_interval == 0:
                    print(
                        f"[WARN] step={step}/{len(train_loader)} 出现非法 loss={loss.item()}，"
                        f"已跳过该 batch（累计跳过 {nonfinite_loss_steps} 次）。"
                    )
                continue

            if use_grad_scaler:
                scaler.scale(loss).backward()
                # 梯度裁剪前需要先 unscale，这是 AMP(FP16) 的标准流程。
            ...
            if step % args.log_interval == 0:
                print(
                    ...
                    f"avg_loss={avg_loss:.4f}  skipped_nonfinite={nonfinite_loss_steps}"
                )

        epoch_loss = running_loss / max(1, effective_steps)
        print(
            f"[INFO] Epoch {epoch} 完成，平均 Loss: {epoch_loss:.4f} "
            f"(有效 step={effective_steps}, 跳过={nonfinite_loss_steps})"
        )

        # 每个 epoch 独立保存一个文件夹，便于阶段性回溯与对比
        epoch_dir_name = get_epoch_folder_name(args.encoder_type, args.training_stage, epoch)
        epoch_ckpt_dir = checkpoint_stage_dir / epoch_dir_name
        epoch_ckpt_dir.mkdir(parents=True, exist_ok=True)

        # 统一保存“自定义时序侧权重”，与阶段目录结构保持一致
        custom_ts_weights = {
            "epoch": epoch,
        ...
        torch.save(custom_ts_weights, epoch_ckpt_dir / "custom_ts_weights.pth")

        if args.training_stage == "lora":
            maybe_save_lora_adapter(model, epoch_ckpt_dir)

        print(f"[INFO] 已保存权重: {epoch_ckpt_dir}")

    if args.preview_after_train:
        try:
